...
OPTIMIZER_CLS_NAMES = {
  ...
  "AdaDelta": train.AdadeltaOptimizer,
}
...
def optimize_loss(loss,
  ...
  """
  Copied From layers.optimizer.py
  Return Gradient
  ...
  with vs.variable_scope(name, "OptimizeLoss", [loss, global_step]):
    ...
    if learning_rate is not None and learning_rate_decay_fn is not None:
      ...
      if "learning_rate" in summaries and report_summary:
    ...
    if ("global_gradient_norm" in summaries or "gradient_norm" in summaries) and report_summary:
      summary.scalar("global_norm/gradient_norm",
                     clip_ops.global_norm(list(zip(*gradients))[0]))

    # Optionally clip gradients by global norm.
    if isinstance(clip_gradients, float):
      gradients_clip = _clip_gradients_by_norm(gradients, clip_gradients)
    elif callable(clip_gradients):
      gradients_clip = clip_gradients(gradients)
    elif clip_gradients is not None:
      raise ValueError(
        "Unknown type %s for clip_gradients" % type(clip_gradients))
    else:
      gradients_clip = gradients

    # Add scalar summary for loss.
    if "loss" in summaries and report_summary:
    ...
    for gradient, variable in gradients_clip:
      ...
      if grad_values is not None:
        ...
        if "gradients" in summaries and report_summary:
          summary.histogram("gradients/%s" % var_name, grad_values)
        if "gradient_norm" in summaries and report_summary:
          summary.scalar("gradient_norm/%s" % var_name,
                         clip_ops.global_norm([grad_values]))

    if clip_gradients is not None and ("global_gradient_norm" in summaries or
                                       "gradient_norm" in summaries) and report_summary:
    ...
    # Output gradient/variable norm to draw on kmonitor
    diy_out_gradient_norm = {}
    diy_out_var_norm = {}
    for gradient, variable in gradients:
      ...
      if grad_values is not None:
        var_name = variable.name.replace(":", "_")
        diy_out_gradient_norm["gradient_norm/%s" % var_name] = clip_ops.global_norm([grad_values])
      if var_values is not None:
        var_name = variable.name.replace(":", "_")
        diy_out_var_norm["variable_norm/%s" % var_name] = clip_ops.global_norm([var_values])
    return train_tensor, diy_out_gradient_norm, diy_out_var_norm
